...
public class SwerveModule {
    ...
    public SwerveModuleState getState() {
        return new SwerveModuleState(m_driveEncoder.getVelocity(),
                Rotation2d.fromDegrees(m_moduleSteeringEncoder.getPosition()));
    }
    ...
    public void setDesiredState(SwerveModuleState desiredState) {
        ...
        Rotation2d rotationDelta = state.angle.minus(currentRotation); // takes our current rotatation and subtracts the
                                                                       // last state rotation

        double deltaTicks = (rotationDelta.getDegrees() / 360) * SwerveDriveModuleConstants.kEncoderTicksPerRotation;
        double currentTicks = m_moduleSteeringEncoder.getPosition()
                / m_moduleSteeringEncoder.configGetFeedbackCoefficient();
